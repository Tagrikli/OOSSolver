...
SEED = 0
INIT_WEIGHTS = "runs/_rescue/curric_v3_SOLVES_ALL.pt"   # robust dig base

REWARD_SUCCESS = 15.0
PENALTY_ALL_WAIT = 1.0
PUTBACK_FRAC = 0.25            # constructed put-back (slack<0) — the sampler rarely makes it
...
def make_sampler_env(max_steps):
    """THE REAL distribution: omni sampler — 0-2 preloaded cars, 0-3 requests
    (multi-task), targets on ANY shelf (small + big), fresh fullness each episode."""
    return RetrieveEnv(
        facility_factory=get_facility(FACILITY),
        room_car_amounts=(0, 1, 2), request_car_amounts=(0, 1, 2, 3), depths=(0, 1, 2),
...
def main():
    ...
    collectors = [make_collector(
        make_putback_env(curric, MAX_EP_STEPS) if i < n_pb else make_sampler_env(MAX_EP_STEPS),
        seed=SEED + i * 1_000_000) for i in range(N_ENVS)]

    print(f"[{RUN_NAME}] device={DEVICE} REAL sampler + {n_pb}/{N_ENVS} put-back | success={REWARD_SUCCESS}")
    t0 = time.time()
    for it in range(1, TOTAL_ITERS + 1):
        ...
        if it % EVAL_EVERY == 0 or it == 1:
            ...
            row["real"] = ev
            print(f"it {it:4d} | REAL {ev} | ret {row['ret']} ent {m.entropy:.2f} | {time.time()-t0:.0f}s")
        else:
