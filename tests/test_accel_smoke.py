...
def _cfg(**over) -> ACCELConfig:
    base = dict(
        ...
        mutation_parents=2,
        edit_steps=2,
        require_solvable=False,  # mutation tests don't need solvability
        mutation_ops=("swap_contents", "shuffle_shelf", "repick_target"),
        metric_top_k=3,
        metric_min_visits=1,
    )
    base.update(over)
...
def test_mutation_admits_neighbors():
    ...
    rng = np.random.default_rng(0)
    added = t.maybe_mutate(it=1, rng=rng, topology=_build_facility().topology)
    assert added > 0
...
def test_mutate_noop_when_buffer_empty():
    ...
    rng = np.random.default_rng(0)
    assert t.maybe_mutate(it=1, rng=rng, topology=_build_facility().topology) == 0
