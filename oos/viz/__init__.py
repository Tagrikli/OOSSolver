...
Three layers, one-way dependency (oos.sim → Session → app):
  ...
  * `Session`     — headless logic: owns the Environment + the plan solver
                    (via SolverBridge), drives playback, and exposes the two
                    interaction surfaces (World pokes + Playback). No
                    DearPyGui imports.
...
the *solver* owns the carriers; the canvas only ever renders sim truth at the
